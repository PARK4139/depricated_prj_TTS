echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>pip 을 인식 못함 error 발생>환경변수 영구추가
set PATH_THAT_I_WANT_TO_ADD=C:\Users\WIN10PROPC3\AppData\Local\Programs\Python\Python311\Scripts
setx path "%path%;%PATH_THAT_I_WANT_TO_ADD%;"
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>project_py 이동
cd C:\Users\WIN10PROPC3
mkdir prjs
cd C:\Users\WIN10PROPC3\prjs
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>pyVirtualEnvironment1 활성화
python -m venv pyVirtualEnvironment1
.\pyVirtualEnvironment1\Scripts\activate
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>library 설치
pip install gTTS
pip install playsound
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>project_TTS_STT 이동
mkdir project_TTS_STT
cd project_TTS_STT
echo ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>python 실행
py


print(" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>jung hoon park library ")
def cls():
    import os
    os.system('cls')
